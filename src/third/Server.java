...
public class Server {
    ...
    private static class MyHandler implements Runnable {
        ...
        public void run() {
            ...
            try {
                ...
                while (true) {
                    ...
                    int automic = bufferedReader.readInt();
                    int len = bufferedReader.readInt();
                    byte[] buffer = new byte[len];
                    bufferedReader.read(buffer);
                    handlerRequest(automic, bufferedWriter, cmd, new String(buffer));
                }
            } catch (IOException e) {
                e.printStackTrace();
            } finally {
        ...
        private void handlerRequest(int automic, DataOutputStream bufferedWriter, int cmd, String json) {
            System.out.println(json);
            try {
                JSONObject jsonObject = new JSONObject(json);
                if (cmd == 1) {
                    bufferedWriter.writeInt(cmd);
                    bufferedWriter.writeInt(automic);
                    bufferedWriter.writeInt("{\"price\":56}".getBytes().length);
                    bufferedWriter.write("{\"price\":56}".getBytes());
                    bufferedWriter.flush();
                } else if (cmd == 3) {
                    time.schedule(new MyTimerTasker(bufferedWriter, cmd, automic), 500, 500);
                } else {
                    bufferedWriter.writeInt(cmd);
                    bufferedWriter.writeInt(automic);
    ...
    public static class MyTimerTasker extends TimerTask {
        ...
        public MyTimerTasker(DataOutputStream dataOutputStream, int cmd, int automic) {
        ...
        public void run() {
            try {
                ...
                dataOutputStream.writeInt(automic);
